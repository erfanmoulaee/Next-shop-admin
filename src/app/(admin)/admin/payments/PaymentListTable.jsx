          ...
          {payments.map((payment, index) => {
            return (
              <tr key={payment._id}>
                ...
                <td className="table__td font-bold text-lg">{toPersianNumbersWithComma(payment.amount)}</td>
                <td className="table__td">{toLocalDateStringShort(payment.createdAt)}</td>
                <td className="table__td">{payment.status === "COMPLETED" ? <span className="badge badge--success">موفق</span> : <span className="badge badge--error">ناموفق</span>}</td>
                <td>
                  <div>
                    <Link href={`/admin/payments/${payment._id}`} className="flex justify-center">
